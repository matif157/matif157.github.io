...
(function () {
  ...
  if (net && !REDUCED) {
    ...
    function spawnBurst(x, y) {
      var count = 18 + Math.floor(Math.random() * 12);
      for (var i = 0; i < count; i++) {
        var angle = Math.random() * 6.283;
    ...
    function step() {
      ...
      for (var i = 0; i < parts.length; i++) {
        ...
        for (var b = 0; b < bursts.length; b++) {
          var bp = bursts[b];
          var bdx = bp.x - p.x, bdy = bp.y - p.y;
          var bmd = Math.sqrt(bdx * bdx + bdy * bdy);
          if (bmd < 80 && bmd > 0.001) {
            var force = (80 - bmd) / 80 * 0.8;
            p.vx -= (bdx / bmd) * force;
            p.vy -= (bdy / bmd) * force;
          }
        }
        for (var r = 0; r < rings.length; r++) {
          var ring = rings[r];
          var rdx = ring.x - p.x, rdy = ring.y - p.y;
          var rmd = Math.sqrt(rdx * rdx + rdy * rdy);
          var thickness = 40;
          if (rmd > ring.r - thickness && rmd < ring.r + thickness && ring.life > 0) {
            var force = (thickness - Math.abs(rmd - ring.r)) / thickness * 1.2;
            p.vx += (rdx / (rmd || 0.001)) * force;
            p.vy += (rdy / (rmd || 0.001)) * force;
          }
        }
      ...
      for (var b = bursts.length - 1; b >= 0; b--) {
        ...
        bp.vy += 0.02;
        bp.life -= 1 / 60;
        var a = Math.max(0, bp.life / bp.maxLife);
        if (a <= 0) { bursts.splice(b, 1); continue; }
        ctx.beginPath();
        ctx.arc(bp.x, bp.y, bp.r * a, 0, 6.2832);
        ctx.fillStyle = 'hsla(' + bp.hue + ', 90%, 60%, ' + (a * 0.8).toFixed(3) + ')';
        ctx.fill();
      }
      for (var r = rings.length - 1; r >= 0; r--) {
        ...
        ring.r += ring.maxR / 45;
        ring.life -= 1 / 45;
        if (ring.life <= 0) { rings.splice(r, 1); continue; }
        var a = ring.life * 0.5;
        ctx.beginPath();
        ctx.arc(ring.x, ring.y, ring.r, 0, 6.2832);
        ctx.lineWidth = ring.width * ring.life;
        ctx.strokeStyle = 'rgba(34,211,238,' + (a * 0.6).toFixed(3) + ')';
        ctx.stroke();
      }
    ...
    d.addEventListener('click', function (e) {
      if (!e.target.closest('a, button, .contact-card, .project, .cap, .metric, .step, .gate-card, .contact-wa, .view-btn, [data-mag]')) {
